...
Pokémon Stadium Lite is a realtime multiplayer battle game where two trainers:

1. Join a lobby with a nickname
2. Receive 3 random Pokémon from the catalog
3. Confirm readiness
...
| **Auto-swap** | Defeated Pokémon are replaced automatically |
...
| `assign_pokemon` | — | Request random 3-Pokémon team |
...
| `battle_end` | `{ lobby, winnerPlayerId }` | No Pokémon remaining |
...
```
  ...
  Player A ──assign_pokemon──► 3 random Pokémon assigned
  Player B ──assign_pokemon──► 3 random Pokémon (no overlap)
...
MIT — Pokémon Stadium Lite backend project.
